...
def dropout_layer(state_before, use_noise, rnd):
...
class LSTMModel(object):
    ...
    def init_lstm_params(self):
        ...
        # lstm
        params['lstm_W'] = np.concatenate(
            [self.ortho_weight(), self.ortho_weight(), self.ortho_weight(), self.ortho_weight()], axis=1)
        params['lstm_U'] = np.concatenate(
            [self.ortho_weight(), self.ortho_weight(), self.ortho_weight(), self.ortho_weight()], axis=1)
        params['lstm_b'] = np.zeros((4 * self.lstm_settings['dim'],)).astype(config.floatX)
        # classifier
        params['U'] = 0.01 * np.random.randn(self.lstm_settings['dim'], self.lstm_settings['label_dim']).astype(
            config.floatX)
    ...
    def adadelta(lr, tparams, grads, x, mask, y, w, cost):
        ...
        zipped_grads = [theano.shared(p.get_value() * np.asarray(0., dtype=config.floatX), name='%s_grad' % k) for k, p
                        in tparams.items()]
        running_up2 = [theano.shared(p.get_value() * np.asarray(0., dtype=config.floatX), name='%s_rup2' % k) for k, p
                       in tparams.items()]
        running_grads2 = [theano.shared(p.get_value() * np.asarray(0., dtype=config.floatX), name='%s_rgrad2' % k) for
                          k, p in tparams.items()]
    ...
    def build_lstm(self):
        ...
        if self.lstm_settings['dropout']:
            proj = dropout_layer(proj, use_noise, rnd)
        ...
        cost = -(w * tensor.log(pred[tensor.arange(n_samples), y] + eps) + (1. - w) * tensor.log(
            1. - pred[tensor.arange(n_samples), y] + eps)).mean()
