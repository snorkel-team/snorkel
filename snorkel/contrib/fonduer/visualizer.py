...
class Visualizer():
    ...
    def display_boxes(self, pdf_file, boxes, page_num=1, alternate_colors=False):
        """
    ...
    def display_candidates(self, candidates, page_num=1, pdf_file=None):
        """
        Displays the bounding boxes corresponding to candidates on an image of the pdf
        boxes is a list of 5-tuples (page, top, left, bottom, right)
        """
        if not pdf_file:
            pdf_file = os.path.join(self.pdf_path, candidates[0][0].parent.document.name + '.pdf')
        boxes = [get_box(span) for c in candidates for span in c.get_arguments()]
        self.display_boxes(pdf_file, boxes, page_num=page_num, alternate_colors=True)

    def display_words(self, phrases, target=None, page_num=1, pdf_file=None):
        if not pdf_file:
            ...
            for i, word in enumerate(phrase.words):
                if target is None or word == target:
                    boxes.append((
                        ...
                        phrase.bottom[i],
                        phrase.right[i]))
        self.display_boxes(pdf_file, boxes, page_num=page_num)
