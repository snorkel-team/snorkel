...
from snorkel.classification import cross_entropy_soft
...
class SoftCrossEntropyTest(unittest.TestCase):
    def test_sce_equals_ce(self):
        ...
        ces_loss = cross_entropy_soft(Y_probs, Y_golds_probs, reduction="none")
        np.testing.assert_equal(ce_loss.numpy(), ces_loss.numpy())

        ce_loss = F.cross_entropy(Y_probs, Y_golds, reduction="sum")
        ces_loss = cross_entropy_soft(Y_probs, Y_golds_probs, reduction="sum")
        np.testing.assert_equal(ce_loss.numpy(), ces_loss.numpy())

        ce_loss = F.cross_entropy(Y_probs, Y_golds, reduction="mean")
        ces_loss = cross_entropy_soft(Y_probs, Y_golds_probs, reduction="mean")
        np.testing.assert_equal(ce_loss.numpy(), ces_loss.numpy())
    ...
    def test_perfect_predictions(self):
        ...
        ces_loss = cross_entropy_soft(Y_probs, Y_golds_probs)
    ...
    def test_lower_loss(self):
        ...
        ces_loss1 = cross_entropy_soft(Y_probs1, Y_golds_probs)
        ces_loss2 = cross_entropy_soft(Y_probs2, Y_golds_probs)
    ...
    def test_loss_weights(self):
        ...
        ces_loss0 = cross_entropy_soft(Y_probs, Y_golds_probs).numpy()
        ces_loss1 = cross_entropy_soft(Y_probs, Y_golds_probs, weight=weight1).numpy()
        ces_loss2 = cross_entropy_soft(Y_probs, Y_golds_probs, weight=weight2).numpy()
        ces_loss3 = cross_entropy_soft(Y_probs, Y_golds_probs, weight=weight3).numpy()
        # Note that PyTorch's cross-entropy loss has the unusual behavior weights
        # behave differently when losses are averaged inside vs. outside the function.
        # See TODO for details.
        ce_loss3 = F.cross_entropy(Y_probs, Y_golds, weight=weight3, reduction="none").mean().numpy()
