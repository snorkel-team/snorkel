...
# class CDRModel(SnorkelPipeline):
#     """
...
#         # fn=tagger_one.tag
#         fn = None
#         SnorkelPipeline.parse(self, doc_preprocessor, fn=fn, clear=clear)
...
#                 SnorkelPipeline.extract(self, candidate_extractor, sents, split=split, clear=clear)
...
#             nCandidates = self.session.query(self.candidate_class).filter(self.candidate_class.split == split).count()
#             if nCandidates > 0:
#                 F = SnorkelPipeline.featurize(self, featurizer, split)
...
#             nCandidates = self.session.query(self.candidate_class).filter(self.candidate_class.split == split).count()
#             if nCandidates > 0:
#                 L = SnorkelPipeline.label(self, labeler, split)
#                 if split==TRAIN:
#                     L_train = L
