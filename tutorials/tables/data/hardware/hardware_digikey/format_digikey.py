...
with open("hardware_digikey_gold.csv", "w") as csvoutput:
    ...
    with open("combined_bjt_data.csv", "r") as csvinput:
        ...
        for line in reader:
            ...
            if(len(line) == 28):
                add_index = 1
            if(len(line) == 29):	#Handling Data With Populated Resistor Entries
                add_index = 2
            if len(line) >= 27:		#Most of the entries have length 26, some have 28; entries with less than that are discarded
                pdf_name = line[-1][:-4]
                part_num = line[3]
                polarity = line[14]
